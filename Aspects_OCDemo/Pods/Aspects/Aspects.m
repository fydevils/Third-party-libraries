...
static Class aspect_hookClass(NSObject *self, NSError **error) {
    ...
    // Already subclassed  如果类名有_Aspects_前缀，说明Class已被Hook
    if ([className hasSuffix:AspectsSubclassSuffix]) {
        return baseClass;
    ...
    // Default case. Create dynamic subclass. // 默认则会动态创建一个子类
    ...
    if (subclass == nil) {
        ...
        aspect_swizzleForwardInvocation(subclass);//实现替换当前类forwardInvocation方法的实现为__ASPECTS_ARE_BEING_CALLED__
        aspect_hookedGetClass(subclass, statedClass); //实现当前类的isa指针指向原生的类
        aspect_hookedGetClass(object_getClass(subclass), statedClass); //实现当前类的元类的isa指针指向原生的类
        objc_registerClassPair(subclass);//注册当前类
    }
    
    object_setClass(self, subclass); //将当前对象的isa指针指向刚生成的类
    return subclass;
}
